...
function UploadFile({ onUpdate }) {
  ...
  async function FilesUpload() {
    try {
      let result = await axios.post(`http://localhost:4000/api/uploadfiles/${newUser.id}`, data, {
        headers: {
          'x-auth-token': token
        }
      })
    ...
    document.getElementById('uploadFiles-close').click()      
      onUpdate(allfiles)
  }
  return (
    <>
        ...
        <div className="modal-dialog">
          <div className="modal-content">
            ...
            <div className="modal-footer">
              <button type="button" className="btn btn-secondary" data-dismiss="modal" id="uploadFiles-close">Close</button>
